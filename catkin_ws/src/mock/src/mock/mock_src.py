...
def ambient_light1_callback(data):
    ...
    if (data.data == 0):
        ...
        if (data.data < dark_threshold):
            dark_pub = rospy.Publisher('/feedback/dark', Int32, queue_size=10)
            dark_threshold = dark_threshold * 0.9
            dark_pub.publish(1)
...
def mock_main():
    ...
    zone_timer    = [  0,     1,     15,    25,    35,    45,   130]

    weather_presence = [1, 0, 0]
    weather_timer = [55, 75, 130]
    ...
    while not rospy.is_shutdown():
        ...
        if (time == zone_timer[idx]):
            ...
            zone_pub[0].publish(zone_presence[idx-1][0])
            zone_pub[1].publish(zone_presence[idx-1][1])
            
        if (time == weather_timer[widx]):
            weather_pub.publish(weather_presence[widx])
            widx = widx + 1
        else:
            weather_pub.publish(weather_presence[widx-1])
        time = time + 1
        rate.sleep()
